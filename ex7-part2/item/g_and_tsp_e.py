...
class Graph:
    ...
    def calculate_travel_cost(self, city_names):
        ...
        :param city_names: List of city names representing the path.
        :return: Total travel cost if the path is valid, None otherwise.
        """
        total_cost = 0
        ...
        for i in range(len(city_names) - 1):
            ...
            # Find the transition (edge) between the current and next city
            current_vertex = self.__vertices[current_city]
            transition = next(
                (t for t in current_vertex.transitions if t.target.name == next_city), None
            )

            if transition is None:
                return None  # No direct edge between the cities
...
while added_edges < num_edges:
    ...
    graph.add_edge(v2_name, v1_name, weight)  # Add the reverse edge
    added_edges += 1
...
# Test the method travel cost
print(f"Travel cost for {valid_path_1}: {graph.calculate_travel_cost(valid_path_1)}")  # Expected: Total cost
print(f"Travel cost for {valid_path_2}: {graph.calculate_travel_cost(valid_path_2)}")  # Expected: Total cost
print(f"Travel cost for {invalid_path_1}: {graph.calculate_travel_cost(invalid_path_1)}")  # Expected: None
print(f"Travel cost for {invalid_path_2}: {graph.calculate_travel_cost(invalid_path_2)}")  # Expected: None
